...
df = pd.read_csv("./static/crop_calendar.csv")
...
async def filter_data(
    ...
    # 문자열에서 공백, 작은따옴표, 괄호를 제거
    # new_path = f
    files = [f for f in os.listdir("./weather") if f"{crop}_{country}_{region}".translate(trans_table) in f]
    filtered_files = []
    for file in files:
