...
def export_zonal_statistics(nirv, mask, feature, out_file, year=None):
    ...
    if (not os.path.exists(out_file)) or (year == current_year):
        nirv_masked = (nirv
                       .map(lambda img: update_mask(img, mask))
                       .filterBounds(feature)
                       .toBands())
        geemap.zonal_statistics(nirv_masked,
                                feature,
                                out_file,
        ...
        for state in states:
            try:
                ...
                for year in average_years:
                    ...
                    asset_id = f'users/wodudqkqh123/USA_{crop}/{year}_{crop}_{state}'
                    mask = ee.Image(asset_id)
