...
const baseStyle = {
  color: "#666",
  weight: 1,
  fill: false  // ✅ 내부 채우지 않음
};

const highlightStyle = {
  color: "#e63946",
  weight: 3,
  fill: false  // ✅ 강조 폴리곤도 선만 표시
};
// 국가 선택 시 전체 경계 표시
document.getElementById("country").addEventListener("change", () => {
  ...
  fetch(`/maps/api/boundaries/?country=${country}`)
    .then(res => res.json())
    .then(data => fetch(data.geojson_url))
    .then(res => res.json())
    .then(geojson => {
...
document.getElementById("state").addEventListener("change", () => {
  ...
  fetch(`/maps/api/state-boundary/?country=${country}&state=${state}`)
    .then(res => res.json())
    .then(data => fetch(data.geojson_url))
    .then(res => res.json())
    .then(geojson => {
